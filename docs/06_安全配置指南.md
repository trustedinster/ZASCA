...
2c2a 采用多层次安全防护体系，从业务应用到基础设施提供全方位安全保障。
...
class ProductForm(forms.ModelForm):
    name = forms.CharField(
        ...
        validators=[
            RegexValidator(
                regex=r'^[a-zA-Z0-9\-\u4e00-\u9fa5]+$',
                message='产品名称只能包含字母、数字、连字符和中文字符'
            )
        ]
    )
    
    display_hostname = forms.CharField(
        max_length=255,
        validators=[
            RegexValidator(
                regex=r'^[a-zA-Z0-9\-\.]+$',
                message='显示地址只能包含字母、数字、连字符和点号'
            )
        ]
    )
