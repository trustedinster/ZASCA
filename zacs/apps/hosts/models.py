...
class Host(models.Model):
    ...
    # 证书验证相关字段
    server_cert_validation = models.CharField(
        max_length=20,
        choices=[('ignore', '忽略证书'), ('validate', '验证证书')],
        default='ignore',
        verbose_name='证书验证模式'
    )
    ca_cert_path = models.CharField(max_length=500, blank=True, verbose_name='CA证书路径')
    client_cert_path = models.CharField(max_length=500, blank=True, verbose_name='客户端证书路径')
    client_key_path = models.CharField(max_length=500, blank=True, verbose_name='客户端私钥路径')
    ...
    def get_connection_client(self):
        ...
        if self.connection_type == 'winrm':
            ...
            return WinrmClient(
                ...
                ca_trust_path=self.ca_cert_path,
                client_cert_pem=self.client_cert_path,
                client_cert_key=self.client_key_path
            )
